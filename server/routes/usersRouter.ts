...
import { Deleted, RegisterUser, User, UserDocument } from '../types';
			...
			for ( const key in update ) {
				if ( update.hasOwnProperty( key ) ) {
					if ( key === 'id' || key === '_id' ) {
						...
						return res.status( STATUS_CODES.BAD ).json( { error: "You can't change sections" } );
					}
...
usersRouter.get('/', auth.required,(req, res) => res.send((req as any).payload))